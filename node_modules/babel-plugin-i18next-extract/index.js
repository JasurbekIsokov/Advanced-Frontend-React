...
var i18next__namespace = /*#__PURE__*/_interopNamespace(i18next);
...
function getDefaultValue(key, locale, config // eslint-disable-next-line @typescript-eslint/no-explicit-any
) {
...
function exportTranslationKeys(keys, locale, config, cache) {
  ...
  for (const [filePath, keysForFilepath] of Object.entries(keysPerFilepath)) {
    ...
    cache.originalTranslationFiles[filePath] = deepmerge__default["default"]((_cache$originalTransl = cache.originalTranslationFiles[filePath]) !== null && _cache$originalTransl !== void 0 ? _cache$originalTransl : {}, loadTranslationFile(exporter$1, config, filePath), {
    ...
    const directoryPath = nodePath__default["default"].dirname(filePath);
    fs__default["default"].mkdirSync(directoryPath, {
      recursive: true
    });
    fs__default["default"].writeFileSync(filePath, exporter$1.stringify({
      config,
      file: translationFile
    }), {
      encoding: 'utf8'
...
function computeDerivedKeys(extractedKey, locale, config) {
  ...
  if (parsedOptions.hasCount) {
    ...
    let pluralCategories; // See https://www.i18next.com/translation-function/plurals#how-to-find-the-correct-plural-suffix
    ...
    const pluralRule = i18n.services.pluralResolver.getRule(locale, // TODO: a comment hint should allow to override cardinality/ordinality.
    ...
    if (config.enableExperimentalIcu) {
      ...
      keys = keys.reduce((accumulator, k) => [...accumulator, ...pluralSuffixes.map(suffix => ({ ...k,
  ...
  return {
    ...
    visitor: {
      Program(path, state) {
        ...
        const opts = state.file.opts; // Check if the current file is excluded.
        ...
        if (BabelTypes__namespace.isFile(path.container)) {
